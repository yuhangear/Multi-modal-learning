...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        if [ "${feats_type}" = raw ]; then
            ...
            #"${train_set}" "${valid_set}" ${test_sets}
            for dset in dev_other ; do
...
if true ; then
    if true ; then
        if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
            ...
            # shellcheck disable=SC2086
           #./slurm.pl --quiet JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
        ...
        if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
            ...
            #  ./slurm.pl --quiet --gpu 1  --num-threads 10 --exclude=node0[3-8]  exp/log_lm       ${python}  espnet2/bin/lm_train.py \
            #         --ngpu "${ngpu}" \
            # shellcheck disable=SC2086
            # ngpu=1
            # num_nodes=1
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type dump/raw/train_clean_100_sp/phone_text_with_id,text_phone,text "
        ...
        #         --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
        #         --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
        #         --valid_data_path_and_name_and_type "dump/raw/dev/phone_text_with_id,text_phone,text" \
        ...
        _opts+="--train_data_path_and_name_and_type dump/raw/train_960_text/text,other_text,text "
        _opts+="--train_data_path_and_name_and_type dump/raw/train_960_text/phone_text_with_id,other_text_phone,text "

        echo ${_opts} ${asr_args}
        echo  --token_list "${token_list}" 
        echo  --output_dir "${asr_exp}"
       # 
        # ./slurm.pl --quiet --gpu 1  --num-threads 4 --exclude=node0[2-9]  exp/log_start_33  ${python}     /home3/yuhang001/espnet/egs2/librispeech_100/letter_bid_fine/espnet2/bin/asr_train.py \
        
        cuda_cmd="/home/asrxiv/w2021/project/eng_man_malay8k/utils/slurm.pl --quiet --num-threads 6 --exclude=node0[2,3,4,5,6,7,8,9]"
        ${python}  -m espnet2.bin.launch \
            --cmd "${cuda_cmd}" \
            --log "${asr_exp}"/train1.log \
            --ngpu "${ngpu}" \
            --num_nodes "${num_nodes}" \
            --init_file_prefix "${asr_exp}"/.dist_init_ \
            --multiprocessing_distributed true -- \
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            ./slurm.pl --quiet --gpu 1  --num-threads 20 --exclude=node0[1,3,4,5,6,7,8,9]   JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
