...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.phone_embedding1 = torch.nn.Embedding(
            num_embeddings=89,  # the last for PAD valu衰减直。这里的大小设置的是整个词典的大小。
            embedding_dim=256,
          )
        ...
        self.line_phone1=torch.nn.Linear(256, 256)
    ...
    def forward(
        ...
        phone_embedding1=torch.nn.functional.dropout( self.line_3phone1(self.sigmoid(self.line_2phone1(self.sigmoid(  self.line_phone1(  self.phone_embedding1.weight   )    )))).unsqueeze(0).repeat(batch_size,1,1) ,p=0.1)
        phone_embedding2=torch.nn.functional.dropout( self.line_3phone2(self.sigmoid(self.line_2phone2(self.sigmoid(  self.line_phone2(  self.phone_embedding2.weight   )    )))).unsqueeze(0).repeat(batch_size,1,1),p=0.1)
        phone_embedding3=torch.nn.functional.dropout( self.line_3phone3(self.sigmoid(self.line_2phone3(self.sigmoid(  self.line_phone3(  self.phone_embedding3.weight   )    )))).unsqueeze(0).repeat(batch_size,1,1),p=0.1)
        phone_embedding4=torch.nn.functional.dropout( self.line_3phone4(self.sigmoid(self.line_2phone4(self.sigmoid(  self.line_phone4(  self.phone_embedding4.weight   )    )))).unsqueeze(0).repeat(batch_size,1,1),p=0.1)
        phone_embedding5=torch.nn.functional.dropout( self.line_3phone5(self.sigmoid(self.line_2phone5(self.sigmoid(  self.line_phone5(  self.phone_embedding5.weight   )    )))).unsqueeze(0).repeat(batch_size,1,1),p=0.1)
        phone_embedding6=torch.nn.functional.dropout( self.line_3phone6(self.sigmoid(self.line_2phone6(self.sigmoid(  self.line_phone6(  self.phone_embedding6.weight   )    )))).unsqueeze(0).repeat(batch_size,1,1),p=0.1)
        phone_embedding7=torch.nn.functional.dropout( self.line_3phone7(self.sigmoid(self.line_2phone7(self.sigmoid(  self.line_phone7(  self.phone_embedding7.weight   )    )))).unsqueeze(0).repeat(batch_size,1,1),p=0.1)
        phone_embedding8=torch.nn.functional.dropout( self.line_3phone8(self.sigmoid(self.line_2phone8(self.sigmoid(  self.line_phone8(  self.phone_embedding8.weight   )    )))).unsqueeze(0).repeat(batch_size,1,1),p=0.1)
        ...
        #得到特征在各个embedding上的分类概率
        attn2_temp1 = self.line_emb1( self.after_norm(intermediate_outs[0][1])) #[2, 303, 89]
        attn2_temp2 = self.line_emb2( self.after_norm(intermediate_outs[0][1]))
        attn2_temp3 = self.line_emb3( self.after_norm(intermediate_outs[0][1]))
        attn2_temp4 = self.line_emb4( self.after_norm(intermediate_outs[0][1]))
        attn2_temp5 = self.line_emb5( self.after_norm(intermediate_outs[0][1]))
        attn2_temp6 = self.line_emb6( self.after_norm(intermediate_outs[0][1]))
        attn2_temp7 = self.line_emb7( self.after_norm(intermediate_outs[0][1]))
        attn2_temp8 = self.line_emb8( self.after_norm(intermediate_outs[0][1]))
        #分类概率，one-hot之后，挑选出embedding
        ...
        #[70, 310, 8, 256],已经是选出来的embedding,接下来需要，softmax
        ...
        #横向，用gumbel挑中，之后，在纵向，用了一下，softmax。
        ...
        final_encoder_out=em_encoder*attn2_temp_mask.transpose(1,2).expand_as(em_encoder) + ~attn2_temp_mask.transpose(1,2).expand_as(encoder_out)*encoder_out
        fina_mask=encoder_mask * attn2_temp_mask
        l2_loss = seqloss.MaskedMSELoss()
        ...
        loss_vq_vae= l2_loss(final_encoder_out, encoder_out.clone().detach() ,mask=fina_mask.transpose(1,2)) 
...
#         loss_vq_vae= l2_loss(final_em_encoder, encoder_out,mask=fina_mask.transpose(1,2)) 
        ...
        if self.use_transducer_decoder:
            ...
            if self.ctc_weight == 0.0:
                ...
                #kaojin_ctc=(0.8* F.mse_loss(self.ctc_phone.ctc_lo( self.after_norm(intermediate_outs[0][1]))  ,self.ctc_phone_other.ctc_lo(self.after_norm(final_encoder_out)).clone().detach() )+ 0.2 * F.mse_loss(self.ctc_phone.ctc_lo( self.after_norm(intermediate_outs[0][1])).clone().detach()   ,self.ctc_phone_other.ctc_lo(self.after_norm(final_encoder_out))))
                kaojin_ctc= F.mse_loss(self.ctc_phone.ctc_lo( self.after_norm(intermediate_outs[0][1]).clone().detach())  ,self.ctc_phone.ctc_lo(self.after_norm(final_encoder_out)) )
                #loss =0.3 * loss_ctc + 0.4 * loss_att + 0.25 *loss_vq_vae + 0.1 * kaojin_ctc
                loss =0.5 * loss_ctc + 0.6*loss_vq_vae + 0.3 * kaojin_ctc
