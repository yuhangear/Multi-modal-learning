...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        if len(speech[0])==5 and speech[0][0]==1.0:
            ...
            # final_encoder_out, chunk_masks  = self.change_embedding4((final_encoder_out,pos_emb),  encoder_mask)
            # final_encoder_out, chunk_masks  = self.change_embedding5((final_encoder_out,pos_emb),  encoder_mask)
            # final_encoder_out, chunk_masks  = self.change_embedding6((final_encoder_out,pos_emb),  encoder_mask)
            # m_temp=final_encoder_out[1]
            # final_encoder_out=self.after_norm(final_encoder_out[0])
    ...
    # ###
            ...
            # loss_ctc, cer_ctc = self._calc_ctc_loss(
            #     final_encoder_out[0], encoder_out_lens, text, text_lengths
            # )
            #建议还是用ctc啊
            # loss_mse_right = self.criterion_phone(
            #     self.ctc_phone.ctc_lo(final_encoder_out[0]), phone_right            )

           

            # # Collect Intermedaite CTC stats
            # stats["loss_interctc_layer{}".format(8)] = loss_mse_right.detach()
...
##################################################################################################
            ...
            text_phone_in_pad,text_phone_out_pad = add_sos_eos(text_phone, 1, 1, 1)
            ...
            #wav和phone进行拼接
            all_encoder=torch.cat((encoder_mid_out,text_phone_embedding),1)
            all_encoder,pos_emb=self.embed(all_encoder)
            #送到后面的网络当中
            text_phone_mask=text_phone_in_pad.ge(1).unsqueeze(1)
            encoder_mask_temp=torch.cat(( encoder_mask,text_phone_mask),2)
            all_encoder, chunk_masks  = self.change_embedding11((all_encoder,pos_emb),  encoder_mask_temp)
 
            all_encoder, chunk_masks  = self.change_embedding22(all_encoder,  encoder_mask_temp)
   
            all_encoder, chunk_masks  = self.change_embedding33(all_encoder,  encoder_mask_temp)
            all_encoder=all_encoder[0]
            old_len=encoder_mid_out.shape[1]
            ...
            # tag2=intermediate_outs[1][1][0]
            # tag3=intermediate_outs[2][1][0]
            # tag4=intermediate_outs[3][1][0]
            # tag5=intermediate_outs[4][1][0]
       
            
            # style_loss=self.style_loss(final_encoder_out,tag2,encoder_mask)+self.style_loss(final_encoder_out,tag3,encoder_mask)+self.style_loss(final_encoder_out,tag4,encoder_mask)+self.style_loss(final_encoder_out,tag5,encoder_mask)
            ...
            if self.ctc_weight == 0.0:
                ...
                loss = 0.3 * loss_ctc + 0.7 * loss_att  + 0.3*loss_other_ctc11 #+ 0.1*(loss_vq_vae+ loss_ic_right)   #0.3 * (loss_vq_vae  + loss_ic_right +style_loss+loss_other_ctc11 )+ 
                # if loss_ic_right<=4:
                #     loss = 0.3 * (loss_vq_vae  + loss_ic_right +style_loss+loss_other_ctc11 )+ 0.3 * loss_ctc + 0.7 * loss_att
                # else:
                #     loss = 0.3* loss_ic_right  + 0.5*loss_other_ctc11  
            # Collect Attn branch stats
            #stats["loss_att"] = loss_att.detach() if loss_att is not None else None
            stats["acc"] = acc_att
            stats["cer"] = cer_att
            ...
            stats["loss_other_ctc11"] = loss_other_ctc11.detach()
            stats["loss_ic_right"] = loss_ic_right.detach()
