...
# from espnet2.text.token_id_converter import TokenIDConverter_phone
...
#         text_name: str = "text",
#         text_phone_name: str = "text_phone",
#     ):
#         super().__init__(train)
#         self.train = train
#         self.speech_name = speech_name
#         self.text_name = text_name
#         self.text_phone_name=text_phone_name
...
#             )
#             self.phone_id_converter = TokenIDConverter_phone(
...
#             #[1289, 90, 2826, 25, 237, 24, 305, 205, 34, 7, 972, 92, 124, 261, ...]
#             data[self.text_name] = np.array(text_ints, dtype=np.int64)
#         #assert check_return_type(data)
#         return data

#     def _phone_process(
#         self, data: Dict[str, Union[str, np.ndarray]],uid
#     ) -> Dict[str, np.ndarray]:
#         if self.text_phone_name in data :
#             text = data[self.text_phone_name]
#             text=text.split()
...
#             num_blank=int(phone_len/16)
#             all_len=phone_len+num_blank
#             blank_pro=num_blank*1.0/all_len
#             new_text=[]
       
#             for i in text:
#                 if random.random() <= blank_pro:
#                     new_text.append("<blank>")
...
#                     new_text.append(self.phone_for_random[ran])  
...
#             #只有纯文本数据要处理时，才mask一下数据
#             if uid.startswith("other"):
#                 text=new_text


#             #text = self.text_cleaner(text)
#             #tokens = self.tokenizer.text2tokens(text)
#             text_ints = self.phone_id_converter.tokens2ids(text)
#             data[self.text_phone_name] = np.array(text_ints, dtype=np.int64)
#         assert check_return_type(data)
#         return data
...
#         data = self._phone_process(data,uid)
...
class CommonPreprocessor(AbsPreprocessor):
    ...
    #text = self.text_cleaner(text)
            ...
            for i in text:
                if random.random() <= blank_pro:
                    new_text.append("<blank>")
                    new_text_right.append("<blank>")

                if random.random() <= 0.02:
                    new_text.append("<blank>")
                    new_text_right.append(i)
                
                elif random.random() <= 0.08:
                    ran=random.randint(0, 78)
                    new_text.append(self.phone_for_random[ran])  
                    new_text_right.append(i)
            ...
            #tokens = self.tokenizer.text2tokens(text)
            text_ints = self.phone_id_converter.tokens2ids(text)
            text_ints_right = self.phone_id_converter.tokens2ids(text_right)

            data[self.text_phone_name] = np.array(text_ints, dtype=np.int64)
            data["phone_right"] = np.array(text_ints_right, dtype=np.int64)
        assert check_return_type(data)
        return data
    def _phone_process_old(
        self, data: Dict[str, Union[str, np.ndarray]],uid
    ) -> Dict[str, np.ndarray]:
        if self.text_phone_name in data :
            text = data[self.text_phone_name]
