...
from espnet2.torch_utils.load_pretrained_model import load_pretrained_model2
...
class AbsTask(ABC):
    ...
    def build_optimizers(
        ...
        if args.sharded_ddp:
            ...
            # decoder_p = list(map(id, model.decoder.parameters()))
            # encoder_10 = list( map(id, model.encoder.encoders[10].parameters()) )
            # encoder_11 = list(map(id, model.encoder.encoders[11].parameters()) )
            # ctc_p=list(map(id, model.ctc.parameters()))

            # base_params = filter(lambda p: id(p) not in   decoder_p +encoder_10 + encoder_11 + ctc_p ,
            #                     model.parameters())
            ...
            #         {'params': model.encoder.encoders[10].parameters(), 'lr': lr * 0.01 },
            #         {'params': model.encoder.encoders[11].parameters(), 'lr': lr * 0.01 },
            #         {'params': model.ctc.parameters(), 'lr': lr * 0.01},
            ...
            for k,v in model.named_parameters():
                if str(k).startswith("decoder.decoders.0.src_attn") or str(k).startswith("decoder.decoders.1.src_attn") or str(k).startswith("decoder.decoders.2.src_attn") or str(k).startswith("decoder.decoders.3.src_attn") or str(k).startswith("decoder.decoders.4.src_attn") :
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        #计算量输入数据的长度和维数，累积丰功、平方值和白变长度
            ...
            #     )
            first_train=False
            model_dir="exp/asr_conformer_lr2e-3_warmup15k_amp_nondeterministic_phone_ctc/valid.acc.ave_10best.pth"
            model_dir="exp/asr_conformer_lr2e-3_warmup15k_amp_nondeterministic_phone_ctc/valid.acc.ave_10best.pth"
            if first_train:
                load_pretrained_model2(
                    model=model,
                    init_param=model_dir,
                    ignore_init_mismatch=args.ignore_init_mismatch,
                ...
                # for p in args.init_param:
                #     logging.info(f"Loading pretrained params from {p}")
                #/home3/yuhang001/espnet/egs2/librispeech_100/lm_pre_letter/lm_pre_best_10.pth
                #
                # load_pretrained_model(
                #     model=model,
                #     init_param="/home3/yuhang001/espnet/egs2/librispeech_100/lm_pre_letter/exp/asr_conformer_lr2e-3_warmup15k_amp_nondeterministic/valid.acc.ave_10best.pth",
                #     ignore_init_mismatch=args.ignore_init_mismatch,
                #     # NOTE(kamo): "cuda" for torch.load always indicates cuda:0
                #     #   in PyTorch<=1.4
                #     map_location=f"cuda:{torch.cuda.current_device()}"
                ...
                # load_pretrained_model(
                #     model=model,
                #     init_param="/home3/yuhang001/espnet/egs2/librispeech_100/letter_fine_lm/exp/asr_conformer_lr2e-3_warmup15k_amp_nondeterministic_right/valid.acc.ave_11best.pth",
                #     ignore_init_mismatch=args.ignore_init_mismatch,
                #     # NOTE(kamo): "cuda" for torch.load always indicates cuda:0
                #     #   in PyTorch<=1.4
                #     map_location=f"cuda:{torch.cuda.current_device()}"
