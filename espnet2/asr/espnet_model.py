    ...
    def forward(
        ...
        if False:
            ...
            if train_text_encoder:
                ...
                #text_phone_in_pad进行mask
                dtext_phone_in_pad_masked=text_phone_in_pad.clone().detach()
                n = dtext_phone_in_pad_masked.numel()
                m = int(round(n*0.25))

                indices = np.random.choice(n, m, replace=False) # alternative: indices = torch.randperm(n)[:m]
                dtext_phone_in_pad_masked.flatten()[indices] = 0

                mask1=dtext_phone_in_pad_masked.ge(1).unsqueeze(-1).repeat(1,1,256)
                mask2=~dtext_phone_in_pad_masked.ge(1).unsqueeze(-1).repeat(1,1,256)

                zero_phone=torch.flip(text_phone_embedding.clone(),[0])

                text_phone_embedding=(text_phone_embedding*mask1+zero_phone*mask2)
                ...
                loss =  self.criterion_phone( self.ctc_phone.ctc_lo(  text_phone_embedding ) , text_phone_in_pad_flag)


                # Collect total loss stats
                stats["loss"] = loss.detach()
                stats["bert_loss"] = loss.detach()
                ##############################################
...
################
        ...
        # )
            ...
            if self.ctc_weight == 0.0:
                ...
                loss = 0.3 * loss_ctc + 0.7 * loss_att + 0.2*loss_ic   + 0.2*loss_other_ctc1 + 0.1*loss_other_ctc2 # + 0.1* loss_vq_vae #+ 0.2*loss_mse  #+0.1*loss_ic + 0.1*loss_other_ctc1 + 0.01*loss_other_ctc2  + 0.1* loss_vq_vae #+  0.1*(loss_mse )+ 0.1* loss_ctc2 #+ 0.1*(loss_vq_vae+ loss_ic_right)   #0.3 * (loss_vq_vae  + loss_ic_right +style_loss+loss_other_ctc11 )+ 
