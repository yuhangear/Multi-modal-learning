...
class ErrorCalculator(object):
    ...
    def __init__(
        ...
        self.bart.eval()  # disable dropout (or leave in train mode to finetune)
        self.aa=self.bart.bpe
    ...
    def convert_to_char(self, ys_hat, ys_pad):
        ...
        for i, y_hat in enumerate(ys_hat):
            ...
            seq_hat =" ".join( str(idx) for idx in (y_hat[:ymax]%50257).detach().cpu().numpy())
            seq_true =" ".join( str(idx) for idx in y_true.detach().cpu().numpy() if int(idx) != -1)
            seq_hat_text=self.aa.decode(seq_hat)
            seq_true_text=self.aa.decode(seq_true)
